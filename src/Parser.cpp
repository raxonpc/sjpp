...
std::pair<std::string, Parser::FindCode> Parser::find_name() noexcept {
    ...
    while (m_html) {
        ...
        } else if (line.find("KOMENTARZE") !=
                   std::string::npos) { // found comments
...
std::pair<std::string, Parser::ParseCode>
Parser::get_tag_content(const std::string& line) noexcept {
    auto left = line.find('>');
...
void Parser::remove_substr(std::string& line,
                           const std::string& substr) noexcept {
    auto n = substr.length();
    for (auto i = line.find(substr); i != std::string::npos;
         i = line.find(substr))
        line.erase(i, n);
}

std::pair<std::vector<std::string>, Parser::ParseCode>
Parser::parse_meanings(const std::string& content) noexcept {
    if (content.size() <= 3)
...
std::pair<std::vector<Entry>, Code> Parser::parse() noexcept {
    ...
    while (true) {
        ...
        case State::start: {
            ...
            if (find_code == FindCode::found_comment) { // reached comments,
                                                        // successful parsing
                m_state = State::comments;
                continue;
            } else if (find_code ==
                       FindCode::not_found) { // neither comments nor a word
                                              // found, invalid file
        ...
        case State::meaning: {
            ...
            std::transform(
                meanings.cbegin(), meanings.cend(), std::back_inserter(output),
